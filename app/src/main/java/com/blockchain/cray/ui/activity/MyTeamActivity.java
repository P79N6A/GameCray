...
import com.blockchain.cray.ui.adapter.MyTeamAdapter;
...
public class MyTeamActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private  List<CrayMyTeamBean.AttachBean.ListBean> list = new ArrayList<>();
    ...
    private void initRecyclerView() {
        ...
        SlideInBottomAnimationAdapter slideAdapter = new SlideInBottomAnimationAdapter(myTeamAdapter);
        //设置不只第一次有动画
        ...
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE &&
                        mLastVisibleItem + 1 == myTeamAdapter.getItemCount() &&
    ...
    private void initDatas(){
        list.clear();
        mHttpCommand.getCrayTeamList(2,new HttpCmdCallback() {
            @Override
            public void onSucceed(Response response) {
                if (response.isSuccessful()) {
                    try {
                        String jsonString = response.body().string();
                        DebugLog.i("getCrayTeamList,jsonString info: "+jsonString);
                        CrayMyTeamBean crayMyTeamBean = JSON.parseObject(jsonString, CrayMyTeamBean.class);
                        if (crayMyTeamBean.isSuccess()) {
                            List<CrayMyTeamBean.AttachBean.ListBean> dataList =  crayMyTeamBean.getAttach().getList();
                            if (dataList != null) {
